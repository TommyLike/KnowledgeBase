...
## 定位
> OpenShell 是 NVIDIA 为自主 AI Agent（尤其是 Claude Code / Codex / Copilot / Cursor 这类编码 Agent）打造的**安全私有运行时**。它的重心不是"再造一个 Docker 沙箱"，而是解决 Agent 落地的三个真问题：(1) **凭据不能交给 Agent**——由出口网关在网络边界注入 provider 凭据；(2) **执行要能隔离且可迁移**——同一套接口跑在 microVM / Podman / Docker / Kubernetes 四种后端上；(3) **GPU 要能安全共享**——通过 VFIO 直通把 GPU 交给沙箱。在 Agent Runtime / Sandbox 生态中，OpenShell 代表「凭据网关 + 多后端沙箱 + GPU 直通」的一体化路线，与 [`anthropic-experimental/sandbox-runtime`](../../anthropic-experimental/sandbox-runtime/)（本地进程级）、E2B（Firecracker SaaS）形成差异。
...
## 关联
- [`anthropic-experimental/sandbox-runtime`](../../anthropic-experimental/sandbox-runtime/) — 竞品/互补。srt 是本地进程级 OS 沙箱（无网关、无 GPU、无多后端）；OpenShell 是带凭据网关和 GPU 直通的服务端运行时，量级更重、面向团队/集群。
- [`kubernetes-sigs/agent-sandbox`](../../agent-runtime/sandbox/agent-sandbox/) — 同为 K8s Agent 沙箱，但 OpenShell 的 K8s 只是四种驱动之一，且额外提供出口凭据网关。
- [`volcano-sh/agentcube`](../../volcano-sh/agentcube/) — 同类 K8s Agent 沙箱；OpenShell 不依赖特定调度器，靠自有 driver + VFIO 做 GPU。
