...
- [`e2b-dev/infra`](../infra/) — 竞品/互补。E2B 是 Firecracker VM 级隔离（硬件级安全但启动慢），srt 是 OS 级隔离（启动快但隔离度低于 VM）
- [`kubernetes-sigs/agent-sandbox`](../agent-sandbox/) — 竞品/互补。K8s 容器沙箱（集群环境），srt 是本地进程沙箱（开发者环境）
- [`TencentCloud/CubeSandbox`](../CubeSandbox/) — 竞品/互补。云厂商容器沙箱，srt 是本地轻量方案
- [`opensandbox-group/OpenSandbox`](../OpenSandbox/) — 竞品/互补。通用沙箱框架（Docker/K8s），srt 是进程级无容器方案
- [`modelcontextprotocol/modelcontextprotocol`](../../protocol/modelcontextprotocol/) — MCP 协议，srt 的关键应用场景是 MCP Server 沙箱化
